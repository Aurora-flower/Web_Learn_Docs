...
# ---------------------------------------------------------------------------------------
# 分组：
text_group = "apple price is $99,orange price is $88"
# result_group = re.search('.+\$\d+.+\$\d+', text_group)
# print(result_group.group())
result_group = re.search('.+(\$\d+).+(\$\d+)', text_group)
print(result_group.groups())
...
# ---------------------------------------------------------------------------------------
# findall：查找所有满足条件的
text_find = "apple price is $99,orange price is $88"
result_find = re.findall(r'\$\d+', text_find)
print(result_find)

# ---------------------------------------------------------------------------------------
# sub：根据规则替换其他字符串
text_sub = "hello, china,hello world"
# new_text = text_sub.replace(" ", "\t")  # 多个替换,需要多次 replace
new_text = re.sub(r' |,', '\t', text_sub)
print(new_text)
...
# ---------------------------------------------------------------------------------------
# split：根据规则分割字符串
text_slice = "hello HuaYin,hello China"
result_slice = re.split(r' |,', text_slice)
print(result_slice)
